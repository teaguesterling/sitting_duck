...
struct NodeConfig {
	...
	// Constructor
	NodeConfig(uint8_t sem_type = 0, ExtractionStrategy name_strat = ExtractionStrategy::NONE,
	           NativeExtractionStrategy native_strat = NativeExtractionStrategy::NONE, uint8_t node_flags = 0)
	    : semantic_type(sem_type), name_strategy(name_strat), native_strategy(native_strat), flags(node_flags) {
	}
};
