...
public:
    static string ExtractReturnType(TSNode node, const string& content) {
        ...
        for (uint32_t i = 0; i < child_count; i++) {
            ...
            if (strcmp(child_type, "predefined_type") == 0 ||
                strcmp(child_type, "identifier") == 0 ||
                strcmp(child_type, "generic_name") == 0 ||
    ...
    static ParameterInfo ExtractParameter(TSNode node, const string& content) {
        ...
        for (uint32_t i = 0; i < child_count; i++) {
            ...
            } else if (strcmp(child_type, "predefined_type") == 0 ||
                       strcmp(child_type, "generic_name") == 0 ||
                       strcmp(child_type, "nullable_type") == 0 ||
                       strcmp(child_type, "array_type") == 0 ||
                       strcmp(child_type, "qualified_name") == 0) {
...
public:
    ...
    static vector<ParameterInfo> ExtractBaseTypes(TSNode node, const string& content,
        ...
        for (uint32_t i = 0; i < child_count; i++) {
            ...
            if (strcmp(child_type, "base_list") == 0) {
                ...
                for (uint32_t j = 0; j < base_count; j++) {
                    ...
                    string type_name = "";
                    if (strcmp(base_type, "identifier") == 0 ||
                        strcmp(base_type, "generic_name") == 0 ||
                        strcmp(base_type, "qualified_name") == 0) {
                        type_name = ExtractNodeText(base_child, content);
                    } else if (strcmp(base_type, "base_type") == 0 ||
                               strcmp(base_type, "simple_base_type") == 0) {
                        ...
                        for (uint32_t k = 0; k < inner_count; k++) {
                            ...
                            if (strcmp(inner_type, "identifier") == 0 ||
                                strcmp(inner_type, "generic_name") == 0 ||
...
public:
    static vector<ParameterInfo> ExtractLambdaParameters(TSNode node, const string& content) {
        ...
        for (uint32_t i = 0; i < child_count; i++) {
            ...
            if (strcmp(child_type, "parameter_list") == 0) {
                params = CSharpNativeExtractor<NativeExtractionStrategy::FUNCTION_WITH_PARAMS>::ExtractParameterList(child, CSharpNativeExtractor<NativeExtractionStrategy::FUNCTION_WITH_PARAMS>::ExtractNodeText(node, content));
                break;
            } else if (strcmp(child_type, "identifier") == 0) {
                // Single parameter without parentheses: x => x + 1
                ParameterInfo param;
                param.name = CSharpNativeExtractor<NativeExtractionStrategy::FUNCTION_WITH_PARAMS>::ExtractNodeText(child, content);
                if (!param.name.empty()) {
        ...
        for (uint32_t i = 0; i < child_count; i++) {
            ...
            if (strcmp(child_type, "predefined_type") == 0 ||
                ...
                for (uint32_t j = 0; j < var_count; j++) {
                    ...
                    if (strcmp(var_type, "predefined_type") == 0 ||
                        strcmp(var_type, "identifier") == 0 ||
                        strcmp(var_type, "generic_name") == 0 ||
                        strcmp(var_type, "nullable_type") == 0) {
