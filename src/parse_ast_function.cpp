...
static void ParseASTExecute(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	...
	if (!data.parsed) {
		try {
			...
			data.result =
			    UnifiedASTBackend::ParseToASTResult(data.code, data.language, "<inline>", data.extraction_config);
			data.parsed = true;
...
static void ParseASTHierarchicalExecute(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	...
	UnifiedASTBackend::ProjectToHierarchicalTableStreaming(data.result.nodes, output, data.current_row, output_index,
	                                                       data.result.source);
...
void ParseASTFunction::Register(ExtensionLoader &loader) {
	...
	TableFunction parse_ast_func("parse_ast", {LogicalType::VARCHAR, LogicalType::VARCHAR}, ParseASTExecute,
	                             ParseASTBind);
