...
inline string ExtractDartReturnType(TSNode node, const string &content) {
	...
	for (uint32_t i = 0; i < child_count; i++) {
		...
		if (strcmp(child_type, "type_identifier") == 0 || strcmp(child_type, "void_type") == 0 ||
		    strcmp(child_type, "function_type") == 0 || strcmp(child_type, "nullable_type") == 0 ||
	...
	if (!ts_node_is_null(parent)) {
		...
		for (uint32_t i = 0; i < parent_count && i < 10; i++) {
			...
			if (strcmp(sibling_type, "type_identifier") == 0 || strcmp(sibling_type, "void_type") == 0 ||
			    strcmp(sibling_type, "function_type") == 0 || strcmp(sibling_type, "nullable_type") == 0) {
	...
	for (uint32_t i = 0; i < child_count; i++) {
		...
		if (strcmp(child_type, "formal_parameter_list") == 0) {
			...
			for (uint32_t j = 0; j < param_count; j++) {
				...
				if (strcmp(param_type, "formal_parameter") == 0 || strcmp(param_type, "constructor_param") == 0 ||
...
inline vector<string> ExtractDartModifiers(TSNode node, const string &content) {
	...
	for (uint32_t i = 0; i < child_count; i++) {
		...
		if (strcmp(child_type, "abstract") == 0 || strcmp(child_type, "static") == 0 ||
		    strcmp(child_type, "external") == 0 || strcmp(child_type, "covariant") == 0 ||
		    strcmp(child_type, "late") == 0 || strcmp(child_type, "final_builtin") == 0 ||
	...
	if (!ts_node_is_null(parent)) {
		...
		for (uint32_t i = 0; i < parent_count && i < 15; i++) {
			...
			if (strcmp(sibling_type, "abstract") == 0 || strcmp(sibling_type, "static") == 0 ||
			    strcmp(sibling_type, "external") == 0 || strcmp(sibling_type, "base") == 0 ||
			    strcmp(sibling_type, "sealed") == 0 || strcmp(sibling_type, "interface") == 0 ||
			    strcmp(sibling_type, "mixin") == 0 || strcmp(sibling_type, "annotation") == 0) {
...
inline vector<ParameterInfo> ExtractDartParentTypes(TSNode node, const string &content, bool &has_extends,
                                                    bool &has_implements, bool &has_with) {
	...
	for (uint32_t i = 0; i < child_count; i++) {
		...
		if (strcmp(child_type, "superclass") == 0) {
			...
			for (uint32_t j = 0; j < super_count; j++) {
				...
				if (strcmp(super_type, "extends") == 0)
					continue;

				if (strcmp(super_type, "type_identifier") == 0 || strcmp(super_type, "identifier") == 0) {
					...
					for (uint32_t k = 0; k < mixin_count; k++) {
						...
						if (strcmp(mixin_type, "with") == 0 || strcmp(mixin_type, ",") == 0)
							continue;

						if (strcmp(mixin_type, "type_identifier") == 0 || strcmp(mixin_type, "identifier") == 0) {
			...
			for (uint32_t j = 0; j < impl_count; j++) {
				...
				if (strcmp(impl_type, "implements") == 0 || strcmp(impl_type, ",") == 0)
					continue;

				if (strcmp(impl_type, "type_identifier") == 0 || strcmp(impl_type, "identifier") == 0) {
...
inline vector<string> ExtractDartClassModifiers(TSNode node, const string &content, bool has_extends,
                                                bool has_implements, bool has_with, bool is_mixin_declaration = false) {
	vector<string> modifiers;
	...
	for (uint32_t i = 0; i < child_count; i++) {
		...
		if (strcmp(child_type, "abstract") == 0 || strcmp(child_type, "base") == 0 ||
		    strcmp(child_type, "sealed") == 0 || strcmp(child_type, "interface") == 0 ||
...
struct DartNativeExtractor<NativeExtractionStrategy::CLASS_WITH_METHODS> {
	static NativeContext Extract(TSNode node, const string &content) {
		...
		context.modifiers = dart_helpers::ExtractDartClassModifiers(node, content, has_extends, has_implements,
		                                                            has_with, is_mixin_declaration);
...
struct DartNativeExtractor<NativeExtractionStrategy::VARIABLE_WITH_TYPE> {
	static NativeContext Extract(TSNode node, const string &content) {
		...
		for (uint32_t i = 0; i < child_count; i++) {
			...
			if (strcmp(child_type, "type_identifier") == 0 || strcmp(child_type, "nullable_type") == 0 ||
			    strcmp(child_type, "function_type") == 0 || strcmp(child_type, "inferred_type") == 0) {
