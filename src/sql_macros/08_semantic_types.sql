...
CREATE OR REPLACE TEMPORARY MACRO ast_is_modifier(semantic_type) AS (
    (semantic_type & 2) = 2
);
...
CREATE OR REPLACE TEMPORARY MACRO ast_filter_declarations(ast_table) AS TABLE
    SELECT * FROM ast_table
    WHERE ast_is_declaration(semantic_type);

-- Filter AST table to only statements
CREATE OR REPLACE TEMPORARY MACRO ast_filter_statements(ast_table) AS TABLE
    SELECT * FROM ast_table
    WHERE ast_is_statement(semantic_type);

-- Filter AST table to only expressions
CREATE OR REPLACE TEMPORARY MACRO ast_filter_expressions(ast_table) AS TABLE
    SELECT * FROM ast_table
