...
ASTResult DuckDBAdapter::ProcessStatements(const vector<unique_ptr<SQLStatement>> &statements,
...
void DuckDBAdapter::FlattenNode(const DuckDBASTNode &node, vector<ASTNode> &nodes, int64_t parent_id, uint32_t depth,
                                int64_t &node_counter) const {
