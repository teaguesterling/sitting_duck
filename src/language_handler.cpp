...
void LanguageHandler::SetParserLanguageWithValidation(TSParser *parser, const TSLanguage *language,
                                                      const string &language_name) const {
	if (!language) {
	...
	if (language_version > TREE_SITTER_LANGUAGE_VERSION) {
		throw InvalidInputException(language_name + " grammar ABI version " + std::to_string(language_version) +
		                            " is newer than tree-sitter library version " +
...
string PythonLanguageHandler::ExtractNodeValue(TSNode node, const string &content) const {
	...
	if (node_type == "string" || node_type == "integer" || node_type == "float" || node_type == "true" ||
	    node_type == "false" || node_type == "none") {
		return ExtractNodeText(node, content);
	}
...
string JavaScriptLanguageHandler::ExtractNodeValue(TSNode node, const string &content) const {
	...
	// For literals, extract the actual value
	if (node_type == "string" || node_type == "number" || node_type == "template_string" || node_type == "true" ||
	    node_type == "false" || node_type == "null") {
		return ExtractNodeText(node, content);
	}
	...
	if (normalized == NormalizedTypes::FUNCTION_DECLARATION || normalized == NormalizedTypes::CLASS_DECLARATION ||
		...
		if (node_type == "function_definition") {
			...
			for (uint32_t i = 0; i < child_count; i++) {
				TSNode child = ts_node_child(node, i);
				const char *child_type = ts_node_type(child);
				if (strcmp(child_type, "identifier") == 0 || strcmp(child_type, "type_identifier") == 0) {
					return ExtractNodeText(child, content);
				}
			}
		}
...
string CPPLanguageHandler::ExtractNodeValue(TSNode node, const string &content) const {
	...
	// For literals, extract the actual value
	if (node_type == "string_literal" || node_type == "number_literal" || node_type == "true" || node_type == "false" ||
	    node_type == "null" || node_type == "nullptr") {
		return ExtractNodeText(node, content);
	}
