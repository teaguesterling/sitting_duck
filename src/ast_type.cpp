...
ASTType::ASTType(const string &file_path, const string &language) : file_path(file_path), language(language) {
}
...
ASTType::ASTType(ASTType &&other) noexcept
    : file_path(std::move(other.file_path)), language(std::move(other.language)), nodes(std::move(other.nodes)),
      node_id_to_index(std::move(other.node_id_to_index)), parent_to_children(std::move(other.parent_to_children)),
      tree(other.tree) {
	other.tree = nullptr;
}
...
unique_ptr<ASTType> ASTType::Deserialize(const Value &value) {
	...
	auto ast = make_uniq<ASTType>(source_struct[0].GetValue<string>(), // file_path
