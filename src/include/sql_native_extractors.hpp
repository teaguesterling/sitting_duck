...
struct SQLNativeExtractor<NativeExtractionStrategy::VARIABLE_WITH_TYPE> {
    static NativeContext Extract(TSNode node, const string& content) {
        ...
        try {
            ...
            if (node_type == "column_definition") {
                ...
                context.parameters.clear();
                context.modifiers.clear();
            } else if (node_type == "literal" || node_type == "string_literal" || 
                      node_type == "number_literal" || node_type == "boolean_literal") {
                context.signature_type = "LITERAL";
                context.parameters.clear();
                context.modifiers = SQLNativeExtractor<NativeExtractionStrategy::VARIABLE_WITH_TYPE>::ExtractLiteralModifiers(node, content);
            } else if (node_type == "column") {
                context.signature_type = "COLUMN";
                context.parameters.clear();
                context.modifiers.clear();
            } else if (node_type == "varchar" || node_type == "keyword_varchar" || 
                      node_type == "bigint" || node_type.find("keyword_") == 0) {
                context.signature_type = "TYPE";
                context.parameters.clear();
                context.modifiers = SQLNativeExtractor<NativeExtractionStrategy::VARIABLE_WITH_TYPE>::ExtractTypeModifiers(node, content);
            } else {
                context.signature_type = "";
                context.parameters.clear();
                context.modifiers.clear();
    ...
    static string ExtractColumnType(TSNode node, const string& content) {
        ...
        for (uint32_t i = 0; i < child_count; i++) {
            ...
            if (child_type == "bigint" || child_type == "varchar" || child_type == "int" || 
                child_type == "decimal" || child_type == "timestamp" || child_type == "text" ||
                child_type == "smallint" || child_type == "double" || child_type == "float" ||
                child_type == "char" || child_type == "time" || child_type == "interval") {
