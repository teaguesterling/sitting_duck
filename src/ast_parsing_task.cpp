...
ASTParsingTask::ASTParsingTask(TaskExecutor &executor, ASTParsingState &parsing_state, const idx_t file_idx_start,
                               const idx_t file_idx_end, const idx_t thread_id)
    : BaseExecutorTask(executor), parsing_state(parsing_state), file_idx_start(file_idx_start),
      file_idx_end(file_idx_end), thread_id(thread_id) {
}
...
void ASTParsingTask::ProcessSingleFile(idx_t file_idx) {
	try {
		...
		ASTResult result =
		    parsing_fn(adapter, content, file_language, file_path, parsing_state.peek_size, parsing_state.peek_mode);
