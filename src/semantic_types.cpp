...
string GetSemanticTypeName(uint8_t semantic_type) {
	...
	case PARSER_CONSTRUCT:
		return "PARSER_CONSTRUCT";
	case PARSER_DELIMITER:
		return "PARSER_DELIMITER";
	case PARSER_PUNCTUATION:
		return "PARSER_PUNCTUATION";
	case PARSER_SYNTAX:
		return "PARSER_SYNTAX";

	// RESERVED types
	case RESERVED_FUTURE1:
		return "RESERVED_FUTURE1";
	case RESERVED_FUTURE2:
		return "RESERVED_FUTURE2";
	case RESERVED_FUTURE3:
		return "RESERVED_FUTURE3";
	case RESERVED_FUTURE4:
		return "RESERVED_FUTURE4";
	...
	case METADATA_DIRECTIVE:
		return "METADATA_DIRECTIVE";
	case METADATA_DEBUG:
		return "METADATA_DEBUG";
	...
	case EXTERNAL_FOREIGN:
		return "EXTERNAL_FOREIGN";
	case EXTERNAL_EMBED:
	...
	case LITERAL_STRUCTURED:
		return "LITERAL_STRUCTURED";

	// NAME types
	case NAME_IDENTIFIER:
		return "NAME_IDENTIFIER";
	case NAME_QUALIFIED:
		return "NAME_QUALIFIED";
	case NAME_SCOPED:
		return "NAME_SCOPED";
	case NAME_ATTRIBUTE:
	...
	case PATTERN_DESTRUCTURE:
		return "PATTERN_DESTRUCTURE";
	case PATTERN_COLLECT:
	...
	case COMPUTATION_CALL:
		return "COMPUTATION_CALL";
	case COMPUTATION_ACCESS:
		return "COMPUTATION_ACCESS";
	case COMPUTATION_EXPRESSION:
		return "COMPUTATION_EXPRESSION";
	case COMPUTATION_CLOSURE:
		return "COMPUTATION_CLOSURE";

	// TRANSFORM types
	case TRANSFORM_QUERY:
		return "TRANSFORM_QUERY";
	case TRANSFORM_ITERATION:
		return "TRANSFORM_ITERATION";
	case TRANSFORM_PROJECTION:
		return "TRANSFORM_PROJECTION";
	case TRANSFORM_AGGREGATION:
		return "TRANSFORM_AGGREGATION";
	...
	// EXECUTION types
	case EXECUTION_STATEMENT:
		return "EXECUTION_STATEMENT";
	case EXECUTION_DECLARATION:
		return "EXECUTION_DECLARATION";
	case EXECUTION_STATEMENT_CALL:
		return "EXECUTION_STATEMENT_CALL";
	case EXECUTION_MUTATION:
		return "EXECUTION_MUTATION";

	// FLOW_CONTROL types
	case FLOW_CONDITIONAL:
		return "FLOW_CONDITIONAL";
	case FLOW_LOOP:
		return "FLOW_LOOP";
	case FLOW_JUMP:
		return "FLOW_JUMP";
...
string GetSuperKindName(uint8_t super_kind) {
	switch (super_kind) {
	case META_EXTERNAL:
		return "META_EXTERNAL";
	case DATA_STRUCTURE:
		return "DATA_STRUCTURE";
	case CONTROL_EFFECTS:
		return "CONTROL_EFFECTS";
	case COMPUTATION:
		return "COMPUTATION";
	default:
		return "UNKNOWN_SUPER_KIND";
	}
}
...
uint8_t GetKindCode(const string &name) {
	static unordered_map<string, uint8_t> kind_to_code = {// META_EXTERNAL kinds
...
uint8_t GetSuperKindCode(const string &name) {
	static unordered_map<string, uint8_t> super_kind_to_code = {{"META_EXTERNAL", META_EXTERNAL},
