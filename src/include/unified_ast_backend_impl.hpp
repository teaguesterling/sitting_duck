...
static bool HasBodyChild(TSNode node) {
	...
	static const char *body_types[] = {"block",              // Java, Python, Go, Rust, C#
...
ASTResult UnifiedASTBackend::ParseToASTResultTemplated(const AdapterType *adapter, const string &content,
	...
	while (!stack.empty()) {
		...
		if (!stack.back().processed) {
			...
			ast_node.type_raw = string(ts_type);            // COPY to owned std::string
			...
			if (config.context >= ContextLevel::NORMALIZED) {
				...
				ast_node.name_raw = SanitizeUTF8(raw_name);
			} else {
				ast_node.name_raw = "";
			}
			...
			if (start_byte < content.size() && end_byte <= content.size() && end_byte > start_byte) {
				...
				if (config.peek == PeekLevel::NONE || config.peek_size == 0) {
					...
					if (source_text.length() <= 50) {
						...
						ast_node.peek =
						    SanitizeUTF8(source_text.length() > 80 ? source_text.substr(0, 77) + "..." : source_text);
					} else {
						...
						ast_node.peek =
						    SanitizeUTF8(first_line.length() > 80 ? first_line.substr(0, 77) + "..." : first_line);
					}
				} else if (config.peek == PeekLevel::CUSTOM) {
					// Custom size mode
					uint32_t effective_size = config.peek_size > 0 ? config.peek_size : 120;
					ast_node.peek = SanitizeUTF8(
					    source_text.length() > effective_size ? source_text.substr(0, effective_size) : source_text);
				} else {
...
void PopulateSemanticFieldsTemplated(ASTNode &node, const AdapterType *adapter, TSNode ts_node, const string &content,
	...
	if (node_config) {
		...
		uint8_t base_type = node.semantic_type & 0xFC;
		if (base_type == SemanticTypes::PARSER_DELIMITER ||
		    base_type == SemanticTypes::PARSER_PUNCTUATION) {
		...
		if (config.context >= ContextLevel::NATIVE && node_config->native_strategy != NativeExtractionStrategy::NONE) {
			try {
				...
				node.native =
				    ExtractNativeContextTemplated<AdapterType>(ts_node, content, node_config->native_strategy);
