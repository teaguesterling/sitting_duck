...
// LogicalType for qualified_name column: LIST<STRUCT<semantic_type, name, index>>.
// Each element is one segment of the scope path from outermost to innermost.
LogicalType UnifiedASTBackend::QualifiedNameColumnType() {
	child_list_t<LogicalType> segment_fields;
	segment_fields.push_back({"semantic_type", SemanticTypeLogicalType()});
	segment_fields.push_back({"name", LogicalType::VARCHAR});
	segment_fields.push_back({"index", LogicalType::INTEGER});
	return LogicalType::LIST(LogicalType::STRUCT(std::move(segment_fields)));
}

// Build a single qualified_name LIST<STRUCT> Value from an ASTNode's segments.
// Returns a NULL LIST Value if the node has no qualified_name segments.
Value UnifiedASTBackend::QualifiedNameValue(const vector<QualifiedNameSegment> &segments) {
	auto col_type = QualifiedNameColumnType();
	if (segments.empty()) {
		return Value(col_type);
	}
	auto struct_type = ListType::GetChildType(col_type);
	vector<Value> seg_values;
	seg_values.reserve(segments.size());
	for (const auto &seg : segments) {
		child_list_t<Value> seg_children;
		// semantic_type is stored as UTINYINT but cast to SEMANTIC_TYPE via struct type.
		seg_children.push_back(
		    {"semantic_type", Value::UTINYINT(seg.semantic_type).DefaultCastAs(SemanticTypeLogicalType())});
		seg_children.push_back({"name", Value(seg.name)});
		seg_children.push_back({"index", Value::INTEGER(seg.index)});
		seg_values.push_back(Value::STRUCT(std::move(seg_children)));
	}
	return Value::LIST(struct_type, std::move(seg_values));
}

// LogicalType for the scope column.
//   scope: STRUCT<
//     current BIGINT,
//     function BIGINT,
//     class BIGINT,
...
//   >
// The named slots (current/function/class/module) are populated on every
// node; stack is sparse (only on IS_SCOPE nodes, matching old scope_stack).
LogicalType UnifiedASTBackend::ScopeColumnType() {
	child_list_t<LogicalType> entry_fields;
	entry_fields.push_back({"id", LogicalType::BIGINT});
	entry_fields.push_back({"kind", SemanticTypeLogicalType()});
	auto entry_type = LogicalType::STRUCT(std::move(entry_fields));

	child_list_t<LogicalType> scope_fields;
	scope_fields.push_back({"current", LogicalType::BIGINT});
	scope_fields.push_back({"function", LogicalType::BIGINT});
	scope_fields.push_back({"class", LogicalType::BIGINT});
	scope_fields.push_back({"module", LogicalType::BIGINT});
	scope_fields.push_back({"stack", LogicalType::LIST(entry_type)});
	return LogicalType::STRUCT(std::move(scope_fields));
}

// Build a single scope STRUCT Value from an ASTNode's ScopeInfo.
// -1 in a slot means "no enclosing scope of that kind" and is surfaced as NULL.
// Stack is NULL (list) when the node isn't itself a scope.
Value UnifiedASTBackend::ScopeValue(const ASTNode::ScopeInfo &scope) {
	auto col_type = ScopeColumnType();
	auto &fields = StructType::GetChildTypes(col_type);
	auto stack_list_type = fields[4].second; // LIST<STRUCT<id, kind>>
	auto entry_type = ListType::GetChildType(stack_list_type);

	auto make_bigint_or_null = [](int64_t v) {
		return v < 0 ? Value(LogicalType::BIGINT) : Value::BIGINT(v);
	};

	child_list_t<Value> children;
	children.push_back({"current", make_bigint_or_null(scope.current)});
	children.push_back({"function", make_bigint_or_null(scope.function_scope)});
	children.push_back({"class", make_bigint_or_null(scope.class_scope)});
	children.push_back({"module", make_bigint_or_null(scope.module_scope)});

	if (scope.has_stack && !scope.stack_data.empty()) {
		vector<Value> stack_values;
		stack_values.reserve(scope.stack_data.size());
		...
		children.push_back({"stack", Value(stack_list_type)}); // NULL list
	}
	return Value::STRUCT(std::move(children));
}

// File-local aliases so existing call sites keep their short names.
static LogicalType QualifiedNameColumnType() {
	return UnifiedASTBackend::QualifiedNameColumnType();
}
static Value QualifiedNameValue(const vector<QualifiedNameSegment> &segments) {
	return UnifiedASTBackend::QualifiedNameValue(segments);
}
static LogicalType ScopeColumnType() {
	return UnifiedASTBackend::ScopeColumnType();
}
static Value ScopeValue(const ASTNode::ScopeInfo &scope) {
	return UnifiedASTBackend::ScopeValue(scope);
}
...
vector<LogicalType> UnifiedASTBackend::GetFlatTableSchema() {
	return {
	    ...
	    QualifiedNameColumnType() // qualified_name (LIST<STRUCT<semantic_type, name, index>>)
	};
}
...
LogicalType UnifiedASTBackend::GetASTStructSchema() {
	...
	node_children.push_back(make_pair("qualified_name", QualifiedNameColumnType()));
	...
	node_children.push_back(make_pair("scope", ScopeColumnType()));
...
vector<LogicalType> UnifiedASTBackend::GetHierarchicalTableSchema() {
	...
	structure_children.push_back(make_pair("scope", ScopeColumnType()));
	...
	context_children.push_back(make_pair("qualified_name", QualifiedNameColumnType()));
...
vector<string> UnifiedASTBackend::GetHierarchicalTableColumnNames() {
	return {
	    ...
	    "structure", // STRUCT(parent_id, depth, sibling_index, children_count, descendant_count, scope)
...
vector<LogicalType> UnifiedASTBackend::GetFlatDynamicTableSchema(const ExtractionConfig &config) {
	...
	if (schema_config.context != ContextLevel::NONE) {
		...
		if (schema_config.context >= ContextLevel::NORMALIZED) {
			...
			schema.push_back(QualifiedNameColumnType()); // qualified_name
		}
	...
	if (schema_config.structure != StructureLevel::NONE) {
		...
		if (schema_config.structure >= StructureLevel::FULL) {
			...
			schema.push_back(ScopeColumnType());     // scope
		}
	}
...
LogicalType UnifiedASTBackend::GetHierarchicalStructSchema() {
	...
	tree_structure_children.push_back(make_pair("scope", ScopeColumnType()));
	...
	context_info_children.push_back(make_pair("qualified_name", QualifiedNameColumnType()));
...
void UnifiedASTBackend::ProjectToTable(const ASTResult &result, DataChunk &output, idx_t &current_row,
	...
	// Verify output chunk has correct number of columns (19 with combined scope)
	if (output.ColumnCount() != 19) {
		throw InternalException("Output chunk has " + to_string(output.ColumnCount()) + " columns, expected 19");
	}

	// Column index map after consolidating scope_id + scope_stack → scope (col 14):
	//   0:node_id 1:type 2:name 3:file_path 4:language
	//   5:start_line 6:start_column 7:end_line 8:end_column
	//   9:parent_id 10:depth 11:sibling_index 12:children_count 13:descendant_count
	...
	for (idx_t i = current_row; i < result.nodes.size() && count < max_count; i++) {
		...
		output.SetValue(14, count, ScopeValue(node.scope));

		if (node.peek.empty()) {
			peek_validity.SetInvalid(count);
		} else {
			peek_vec[count] = StringVector::AddString(output.data[15], node.peek);
		}
		...
		// Qualified name (LIST<STRUCT<semantic_type, name, index>>)
		output.SetValue(18, count, QualifiedNameValue(node.name_qualified_segments));
...
void UnifiedASTBackend::ProjectToDynamicTable(const ASTResult &result, DataChunk &output, idx_t &current_row,
	...
	// LIST<STRUCT>; written via SetValue so no direct data pointer.
	idx_t qualified_name_col = 0;
	bool have_qualified_name_col = false;
		...
		if (schema_config.context != ContextLevel::NONE) {
			...
			if (schema_config.context >= ContextLevel::NORMALIZED) {
				if (config.context >= ContextLevel::NORMALIZED) {
					...
					// Qualified name (top-level, LIST<STRUCT>)
					output.SetValue(qualified_name_col, count, QualifiedNameValue(node.name_qualified_segments));
				} else {
					// +schema: column exists but data wasn't computed — NULL
					FlatVector::SetNull(output.data[name_col], count, true);
					output.SetValue(qualified_name_col, count, Value(QualifiedNameColumnType()));
				}
			}
			if (schema_config.context >= ContextLevel::NATIVE) {
				if (config.context >= ContextLevel::NATIVE) {
		...
		if (schema_config.structure != StructureLevel::NONE) {
			...
			if (schema_config.structure >= StructureLevel::FULL) {
				if (config.structure >= StructureLevel::FULL) {
					...
					// scope STRUCT<current, function, class, module, stack>
					output.SetValue(scope_col, count, ScopeValue(node.scope));
				} else {
					// +schema: column exists but data wasn't computed — NULL
					...
					FlatVector::SetNull(output.data[scope_col], count, true);
				}
			}
		}
...
Value UnifiedASTBackend::CreateASTStruct(const ASTResult &result) {
	...
	for (const auto &node : result.nodes) {
		...
		node_children.push_back(make_pair("qualified_name", QualifiedNameValue(node.name_qualified_segments)));
		...
		// scope STRUCT<current, function, class, module, stack>
		node_children.push_back(make_pair("scope", ScopeValue(node.scope)));
	...
	node_schema.push_back(make_pair("qualified_name", QualifiedNameColumnType()));
	...
	node_schema.push_back(make_pair("scope", ScopeColumnType()));
...
void UnifiedASTBackend::ProjectToHierarchicalTable(const ASTResult &result, DataChunk &output, idx_t &current_row,
	...
	for (idx_t i = current_row; i < result.nodes.size() && count < max_count; i++) {
		...
		context_values.push_back(make_pair("qualified_name", QualifiedNameValue(node.name_qualified_segments)));
...
void UnifiedASTBackend::ProjectToHierarchicalTableStreaming(const vector<ASTNode> &nodes, DataChunk &output,
	...
	struct RowData {
		...
		vector<QualifiedNameSegment> qualified_name_segments;
		string annotations;
	...
	for (idx_t i = 0; i < rows_to_process; i++) {
		...
		// Qualified name (top-level, from name_qualified_segments)
		row_data.qualified_name_segments = node.name_qualified_segments;
	...
	for (idx_t i = 0; i < collected_rows.size(); i++) {
		...
		// scope STRUCT<current, function, class, module, stack>
		structure_scope_vec.SetValue(row_idx, ScopeValue(row_data.scope));
...
Value UnifiedASTBackend::CreateHierarchicalASTStruct(const ASTResult &result) {
	...
	for (const auto &node : result.nodes) {
		...
		structure_children.push_back(make_pair("scope", ScopeValue(node.scope)));
		...
		context_children.push_back(make_pair("qualified_name", QualifiedNameValue(node.name_qualified_segments)));
