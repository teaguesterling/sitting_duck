...
struct HashMethod {
    ...
    // Constructor
    HashMethod(HashMethodType t = HashMethodType::STRUCTURAL, 
               const vector<string>& props = vector<string>(), 
               const vector<string>& attrs = vector<string>(), 
...
struct NodeTypeConfig {
    ...
    // Constructor
    NodeTypeConfig(ASTKind k = ASTKind::PARSER_SPECIFIC, 
                   uint8_t super = 0, 
                   uint8_t parser = 0, 
                   const HashMethod& hash = HashMethod::Structural(), 
                   uint8_t flags = 0)
...
public:
    ...
    const NodeTypeConfig* GetNodeConfig(const string& type_name) const {
        ...
        if (StringUtil::EndsWith(type_name, "_declaration") || 
            StringUtil::EndsWith(type_name, "_definition")) {
    ...
    void SetDefaults(const NodeTypeConfig& definition,
                     const NodeTypeConfig& expression,
                     const NodeTypeConfig& statement,
                     const NodeTypeConfig& identifier,
                     const NodeTypeConfig& unknown) {
