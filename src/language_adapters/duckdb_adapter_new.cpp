...
ASTResult DuckDBAdapter::ConvertStatementsToAST(const vector<unique_ptr<SQLStatement>> &statements,
	...
	// Root program node
	ASTNode program_node = CreateASTNode("program", "", "", SemanticTypes::DEFINITION_MODULE, node_counter++, -1, 0);
...
vector<ASTNode> DuckDBAdapter::ConvertStatement(const SQLStatement &stmt, uint32_t &node_counter) const {
	...
	case StatementType::INSERT_STATEMENT: {
		auto node = CreateASTNode("insert_statement", "", stmt.ToString(), SQLSemanticTypes::EXECUTION_MUTATION_INSERT,
		                          node_counter++, -1, 1);
		nodes.push_back(node);
		break;
	}
	case StatementType::UPDATE_STATEMENT: {
		auto node = CreateASTNode("update_statement", "", stmt.ToString(), SQLSemanticTypes::EXECUTION_MUTATION_UPDATE,
		                          node_counter++, -1, 1);
		nodes.push_back(node);
		break;
	}
	case StatementType::DELETE_STATEMENT: {
		auto node = CreateASTNode("delete_statement", "", stmt.ToString(), SQLSemanticTypes::EXECUTION_MUTATION_DELETE,
		                          node_counter++, -1, 1);
		nodes.push_back(node);
		break;
	}
...
vector<ASTNode> DuckDBAdapter::ConvertSelectStatement(const SelectStatement &stmt, uint32_t &node_counter) const {
	...
	auto select_node = CreateASTNode("select_statement", "", stmt.ToString(), SQLSemanticTypes::TRANSFORM_QUERY_SELECT,
	                                 node_counter++, -1, 1);
...
vector<ASTNode> DuckDBAdapter::ConvertSelectNode(const SelectNode &node, uint32_t &node_counter) const {
	...
	// SELECT node
	auto select_node =
	    CreateASTNode("select_node", "", "", SQLSemanticTypes::TRANSFORM_QUERY_SELECT, node_counter++, -1, 2);
	uint32_t select_node_id = select_node.node_id;
	nodes.push_back(select_node);
	...
	if (!node.select_list.empty()) {
		auto list_node =
		    CreateASTNode("select_list", "", "", SemanticTypes::ORGANIZATION_LIST, node_counter++, select_node_id, 3);
...
vector<ASTNode> DuckDBAdapter::ConvertExpression(const ParsedExpression &expr, uint32_t &node_counter) const {
	...
	case ExpressionType::FUNCTION: {
		...
		auto node = CreateASTNode("function_call", func_expr.function_name, func_expr.function_name,
		                          SQLSemanticTypes::COMPUTATION_CALL_FUNCTION, node_counter++, -1, 0);
		nodes.push_back(node);
	...
	case ExpressionType::VALUE_CONSTANT: {
		...
		auto node = CreateASTNode("literal", value, value, SemanticTypes::LITERAL_ATOMIC, node_counter++, -1, 0);
		nodes.push_back(node);
		break;
	}
	default: {
		auto node = CreateASTNode("expression", "", expr.ToString(), SemanticTypes::COMPUTATION_EXPRESSION,
		                          node_counter++, -1, 0);
		nodes.push_back(node);
		break;
	}
...
ASTResult DuckDBAdapter::CreateErrorResult(const string &error_message) const {
	...
	ASTNode error_node = CreateASTNode("parse_error", "error", error_message, SemanticTypes::ERROR_SYNTAX, 0, -1, 0);
