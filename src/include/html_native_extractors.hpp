...
struct HTMLNativeExtractor<NativeExtractionStrategy::FUNCTION_WITH_PARAMS> {
    ...
    static bool IsSemanticElement(const string& tag_name) {
        static const set<string> semantic_tags = {
    ...
    static bool IsFormElement(const string& tag_name) {
        static const set<string> form_tags = {
    ...
    static bool IsInteractiveElement(const string& tag_name) {
        static const set<string> interactive_tags = {
            "a", "button", "input", "select", "textarea", "details", "summary",
            "dialog", "embed", "iframe", "img", "audio", "video"
        };
...
struct HTMLNativeExtractor<NativeExtractionStrategy::VARIABLE_WITH_TYPE> {
    ...
    static string CategorizeAttribute(const string& name) {
        if (IsIdAttribute(name)) return "ID_ATTRIBUTE";
        if (IsClassAttribute(name)) return "CLASS_ATTRIBUTE";
        if (IsEventAttribute(name)) return "EVENT_ATTRIBUTE";
        if (IsDataAttribute(name)) return "DATA_ATTRIBUTE";
        if (IsAriaAttribute(name)) return "ARIA_ATTRIBUTE";
        if (IsUrlAttribute(name)) return "URL_ATTRIBUTE";
        if (IsFormAttribute(name)) return "FORM_ATTRIBUTE";
        if (IsMediaAttribute(name)) return "MEDIA_ATTRIBUTE";
    ...
    static bool IsFormAttribute(const string& name) {
        static const set<string> form_attrs = {
            "name", "value", "type", "placeholder", "required", "disabled",
            "readonly", "checked", "selected", "multiple", "pattern", "min", "max"
        };
        return form_attrs.find(name) != form_attrs.end();
    }
    
    static bool IsMediaAttribute(const string& name) {
        static const set<string> media_attrs = {
            "width", "height", "alt", "controls", "autoplay", "loop", "muted", "poster"
        };
    ...
    static bool IsBooleanValue(const string& value) {
        return value == "true" || value == "false" || value == "checked" || 
