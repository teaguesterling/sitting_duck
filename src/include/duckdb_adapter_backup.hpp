...
private:
	...
	struct DuckDBASTNode {
		...
		DuckDBASTNode(const string &node_type, uint8_t sem_type, uint8_t node_flags = 0)
		    : type(node_type), semantic_type(sem_type), flags(node_flags), start_line(1), start_col(1), end_line(1),
		      end_col(1) {
		}
	};
	...
	void FlattenNode(const DuckDBASTNode &node, vector<ASTNode> &nodes, int64_t parent_id, uint32_t depth,
