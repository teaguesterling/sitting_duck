...
struct CSSNativeExtractor<NativeExtractionStrategy::VARIABLE_WITH_TYPE> {
	...
	static vector<string> ExtractPropertyModifiers(TSNode node, const string &content) {
		...
		string prop_name =
		    CSSNativeExtractor<NativeExtractionStrategy::FUNCTION_WITH_PARAMS>::ExtractNodeText(node, content);
	...
	static vector<string> ExtractNumberModifiers(TSNode node, const string &content) {
		...
		if (!ts_node_is_null(parent)) {
			...
			for (uint32_t i = 0; i < child_count; i++) {
				...
				if (child_type == "unit") {
					string unit = CSSNativeExtractor<NativeExtractionStrategy::FUNCTION_WITH_PARAMS>::ExtractNodeText(
					    child, content);
