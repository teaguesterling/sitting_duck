	...
	if (config && config->name_strategy == ExtractionStrategy::CUSTOM) {
		...
		if (node_type == "table" || node_type == "table_array_element") {
			...
			for (uint32_t i = 0; i < child_count; i++) {
				...
				if (strcmp(child_type, "bare_key") == 0 || strcmp(child_type, "quoted_key") == 0 ||
...
string TOMLAdapter::ExtractNodeValue(TSNode node, const string &content) const {
	...
	if (node_type == "string" || node_type == "integer" || node_type == "float" || node_type == "boolean" ||
	    node_type == "local_date" || node_type == "local_time" || node_type == "local_date_time" ||
	    node_type == "offset_date_time") {
