		...
		if (!ts_node_is_null(parent)) {
			...
			for (uint32_t i = 0; i < parent_count; i++) {
				...
				if (strcmp(sibling_type, "accessibility_modifier") == 0 || strcmp(sibling_type, "readonly") == 0 ||
				    strcmp(sibling_type, "static") == 0 || strcmp(sibling_type, "abstract") == 0 ||
...
public:
	static vector<ParameterInfo> ExtractArrowFunctionParameters(TSNode node, const string &content) {
		...
		for (uint32_t i = 0; i < child_count; i++) {
			...
			if (strcmp(child_type, "formal_parameters") == 0) {
				...
				params = TypeScriptNativeExtractor<
				    NativeExtractionStrategy::FUNCTION_WITH_PARAMS>::ExtractTypeScriptParametersDirect(child, content);
				break;
			} else if (strcmp(child_type, "identifier") == 0) {
	...
	static string ExtractArrowReturnType(TSNode node, const string &content) {
		...
		for (uint32_t i = 0; i < child_count; i++) {
			...
			if (strcmp(child_type, "type_annotation") == 0) {
				return TypeScriptNativeExtractor<
				    NativeExtractionStrategy::FUNCTION_WITH_PARAMS>::ExtractTypeFromAnnotation(child, content);
			}
		}
		return "";
	}
...
public:
	...
	static vector<ParameterInfo> ExtractInterfaceParents(TSNode node, const string &content, bool &has_extends) {
		vector<ParameterInfo> parents;
		has_extends = false;
		...
		for (uint32_t i = 0; i < child_count; i++) {
			...
			if (strcmp(child_type, "extends_type_clause") == 0 || strcmp(child_type, "extends_clause") == 0) {
				has_extends = true;
	...
	static void ExtractTypeIdentifiers(TSNode clause_node, const string &content, vector<ParameterInfo> &parents) {
		...
		for (uint32_t i = 0; i < child_count; i++) {
			...
			// Skip keyword nodes
			if (strcmp(child_type, "extends") == 0 || strcmp(child_type, "implements") == 0 ||
			    strcmp(child_type, ",") == 0) {
				continue;
			}

			if (strcmp(child_type, "type_identifier") == 0 || strcmp(child_type, "generic_type") == 0 ||
			    strcmp(child_type, "nested_type_identifier") == 0 || strcmp(child_type, "member_expression") == 0) {
	...
	static vector<string> ExtractTypeScriptClassModifiers(TSNode node, const string &content, bool has_extends,
	                                                      bool has_implements) {
		vector<string> modifiers;
		...
		for (uint32_t i = 0; i < child_count; i++) {
			...
			if (strcmp(child_type, "abstract") == 0 || strcmp(child_type, "export") == 0 ||
	...
	static vector<string> ExtractInterfaceModifiers(TSNode node, const string &content, bool has_extends) {
		vector<string> modifiers;
		...
		for (uint32_t i = 0; i < child_count; i++) {
			...
			if (strcmp(child_type, "export") == 0 || strcmp(child_type, "declare") == 0) {
...
public:
		...
		if (!ts_node_is_null(parent)) {
			...
			if (!ts_node_is_null(grand_parent)) {
				...
				for (uint32_t i = 0; i < gp_count; i++) {
					...
					if (strcmp(child_type, "const") == 0 || strcmp(child_type, "let") == 0 ||
					    strcmp(child_type, "var") == 0 || strcmp(child_type, "readonly") == 0 ||
					    strcmp(child_type, "export") == 0 || strcmp(child_type, "declare") == 0) {
						uint32_t start = ts_node_start_byte(child);
						uint32_t end = ts_node_end_byte(child);
						if (start < content.length() && end <= content.length()) {
							modifiers.push_back(content.substr(start, end - start));
	...
	static string ExtractTypeFromAnnotation(TSNode annotation_node, const string &content) {
		...
		return TypeScriptNativeExtractor<NativeExtractionStrategy::FUNCTION_WITH_PARAMS>::ExtractTypeFromAnnotation(
		    annotation_node, content);
	}
