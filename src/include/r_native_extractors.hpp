			...
			if (child_type && strcmp(child_type, "call") == 0) {
				...
				if (start < content.length() && end <= content.length()) {
					...
					if (call_text.find("return(") != string::npos) {
						...
						if (call_text.find("TRUE") != string::npos || call_text.find("FALSE") != string::npos) {
							has_logical = true;
						} else if (call_text.find("\"") != string::npos || call_text.find("paste") != string::npos) {
							has_character = true;
						} else if (call_text.find("sum(") != string::npos || call_text.find("mean(") != string::npos ||
