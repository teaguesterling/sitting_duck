...
struct ParameterInfo {
	...
	// Constructor
	ParameterInfo(const string &param_name = "", const string &param_type = "", const string &default_val = "",
	              bool optional = false, bool variadic = false, const string &annot = "{}")
	    : name(param_name), type(param_type), default_value(default_val), is_optional(optional), is_variadic(variadic),
	      annotations(annot) {
	}
};
...
struct NativeContext {
	...
	// Helper constructor
	NativeContext(const string &sig_type, const vector<ParameterInfo> &params = {}, const vector<string> &mods = {},
	              const string &qual_name = "", const string &annot = "{}")
	    : signature_type(sig_type), parameters(params), modifiers(mods), qualified_name(qual_name), annotations(annot) {
	}
};
...
public:
	...
	const vector<ASTNode> &GetNodes() const {
		return nodes;
	}
